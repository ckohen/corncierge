...
class ColorsCommand extends BaseCommand {
  ...
  run(message, args) {
    ...
    if (chan) {
      if (!roleAssign(this.socket, message, chan, colorSnowflakes, args)) {
        this.socket.app.database.tables.colorManager.edit(String(message.guildId), guild.roles, guild.snowflakes);
      }
    } else {
      message.delete();
    }
